#!/usr/bin/env python3
"""Fetch Day-Ahead Energy Prices (A44) from the ENTSO-E Transparency Platform API.

Usage examples:
    uv run scripts/fetch_entsoe_prices.py --areas HU,CZ,SK --start 2021-01-01 --end 2025-12-31
    uv run scripts/fetch_entsoe_prices.py  # interactive mode
    ENTSOE_TOKEN=your-token uv run scripts/fetch_entsoe_prices.py --areas FR
"""
...
def parse_areas(spec: str) -> list[str]:
    """Parse 'HU,CZ,SK' into ['HU', 'CZ', 'SK'], validating each code."""
...
def select_areas_interactive() -> list[str]:
    ...
    print()
    print("Enter numbers (e.g. 1,3,5) or area codes (e.g. HU,CZ), or both:")
...
def parse_prices_xml(root: ET.Element, area: str) -> list[dict]:
    """Parse Publication_MarketDocument XML (A44) into a list of row dicts."""
...
def save_csv(rows: list[dict], area: str, start: date, end: date) -> str:
    filename = f"day_ahead_prices_{area}_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.csv"
    ...
    df = (
        pl.DataFrame(rows)
        .sort("datetime_utc")
        # Keep the lowest-resolution (most granular) entry per timestamp when
        # multiple TimeSeries overlap (e.g. day-ahead + intraday settlement).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch ENTSO-E Day-Ahead Energy Prices (A44) and save as CSV.",
    ...
    parser.add_argument(
        "--areas",
        help="Comma-separated list of area codes, e.g. HU,CZ,SK",
    )
    parser.add_argument(
        "--start",
    ...
    print(f"\nFetching day-ahead prices from {start_date} to {end_date} (UTC)")
    print(f"Areas: {', '.join(areas)}\n")

    for area in areas:
        domain = EIC_CODES[area][0]
        ...
        for batch_start, batch_end in batches:
            ...
            rows = parse_prices_xml(root, area)
            if not rows:
                print("no data")
            else:
