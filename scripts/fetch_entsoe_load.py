#!/usr/bin/env python3
"""Fetch Actual Total Load (A65) from the ENTSO-E Transparency Platform API.

Usage examples:
    uv run scripts/fetch_entsoe_load.py --areas HU,CZ,SK --start 2021-01-01 --end 2025-12-31
    uv run scripts/fetch_entsoe_load.py  # interactive mode
    ENTSOE_TOKEN=your-token uv run scripts/fetch_entsoe_load.py --areas FR
"""
...
def parse_load_xml(root: ET.Element, area: str) -> list[dict]:
    """Parse GL_MarketDocument XML (A65) into a list of row dicts."""
...
def save_csv(rows: list[dict], area: str, start: date, end: date) -> str:
    filename = f"total_load_{area}_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.csv"
    output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data")
    os.makedirs(output_dir, exist_ok=True)
    filepath = os.path.join(output_dir, filename)
    df = pl.DataFrame(rows)
    df = df.sort("datetime_utc").unique(subset=["datetime_utc", "area"], keep="first")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch ENTSO-E Actual Total Load (A65) and save as CSV.",
    ...
    print(f"\nFetching total load from {start_date} to {end_date} (UTC)")
